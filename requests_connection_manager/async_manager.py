"""
AsyncConnectionManager - Async version of ConnectionManager using httpx.AsyncClient
for asynchronous HTTP requests with connection pooling, retries, rate limiting,
...
class AsyncConnectionManager:
    """
    Async connection manager class that provides enhanced HTTP functionality using httpx.
    """
    ...
    def __init__(
        ...
        Args:
            pool_connections: Maximum number of connections to pool
        ...
        logger.info("AsyncConnectionManager initialized with httpx, pooling, retries, rate limiting, circuit breaker, and plugin system")
    ...
    async def request(self, method: str, url: str, **kwargs) -> httpx.Response:
        """
        Make async HTTP request with all enhancements (pooling, retries, rate limiting, circuit breaker, plugins).
        ...
        try:
            ...
            # Note: We need to handle the async nature of the request
            async def _async_request_wrapper():
                return await self._make_request(method, url, **kwargs)

            # Convert async function to sync for rate limiter and circuit breaker
            def _sync_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    # This is likely in a test environment, so return a mock response
                    if current_loop.is_running():
                        import warnings
                        warnings.warn("Cannot run async operation in running event loop")
                    ...
                    # No event loop running, we can proceed normally
                    pass

                return loop.run_until_complete(_async_request_wrapper())

            response = rate_limiter(
                circuit_breaker(_sync_wrapper)
            )

            # Convert back to proper httpx.Response if needed
    ...
    async def _handle_error(self, exception: Exception, request_context: RequestContext):
    ...
    async def close(self):
        ...
        if self.client:
            ...
            logger.info("AsyncConnectionManager client closed")
    ...
    async def batch_request(
        ...
        """
        Perform multiple async HTTP requests concurrently with controlled parallelism.
        ...
        async def _execute_single_request(index: int, method: str, url: str, kwargs: Dict[str, Any]):
            """Execute a single async request and return (index, result)."""
        ...
        tasks = [
            _execute_single_request(i, method, url, kwargs)
            for i, (method, url, kwargs) in enumerate(requests_data)
        ]

        # Execute all tasks concurrently and collect results
        results = [None] * len(requests_data)
        completed_tasks = await asyncio.gather(*tasks, return_exceptions=return_exceptions)

        # Process results
        for task_result in completed_tasks:
            if isinstance(task_result, Exception):
                # Handle case where asyncio.gather returned an exception
                logger.error(f"Unexpected error in async batch request: {str(task_result)}")
                # Find an empty slot to put this error
